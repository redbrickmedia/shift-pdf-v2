...
*/
  ...
  /***/ 116(
    ...
    var findWithoutClosingOnEarlyError =
      iteratorHelperWithoutClosingOnEarlyError('find', TypeError);
    ...
    Context.prototype = {
      ...
      number: function () {
        ...
        if (at(source, i) === '.') {
          ...
          if (fractionStartIndex === i)
            throw new SyntaxError("Failed to parse number's fraction at: " + i);
        }
  ...
  /***/ 6069(
    ...
    var SYNTAX_SOLIDUS = /^[$()*+./?[\\\]^{|}]/;
    var OTHER_PUNCTUATORS_AND_WHITESPACES = RegExp(
      '^[!"#%&\',\\-:;<=>@`~' + WHITESPACES + ']'
    );
          ...
          for (var i = 0; i < length; i++) {
            ...
            if (i === 0 && exec(FIRST_DIGIT_OR_ASCII, chr)) {
              ...
              // unpaired surrogate
                ...
                charCode >= 0xdc00 ||
                i + 1 >= length ||
                (charCodeAt(S, i + 1) & 0xfc00) !== 0xdc00
              )
                result[i] = escapeChar(chr);
              // surrogate pair
              else {
...
var es_typed_array_with = __webpack_require__(9577); // ./web/pdfjs.js
const {
  AbortException,
  ...
  version,
  XfaLayer,
} = globalThis.pdfjsLib; // ./web/pdf_find_utils.js
      ...
      if (c.normalize('NFKC') !== c && !diacriticsRegex.test(c)) {
        ...
        if (range[1] + 1 !== i) {
          if (range[0] === range[1]) {
            ...
            ranges.push(
              `${String.fromCharCode(range[0])}-${String.fromCharCode(range[1])}`
            );
          }
          range[0] = range[1] = i;
        } else {
...
var es_array_includes = __webpack_require__(4423); // ./web/ui_utils.js
...
var web_url_parse = __webpack_require__(5781); // ./web/pdf_link_service.js
...
var es_weak_map_get_or_insert_computed = __webpack_require__(9452); // ./web/base_download_manager.js
...
class PDFHistory {
  ...
  #tryPushCurrentPosition(temporary = false) {
    ...
    if (
      ...
      (false || this._numPositionUpdates <= POSITION_UPDATED_THRESHOLD)
    ) {
      return;
    }
    let forceReplace = false;
...
var es_iterator_find = __webpack_require__(116); // ./web/text_layer_builder.js
...
var es_iterator_some = __webpack_require__(3579); // ./web/pdf_rendering_queue.js
...
class PDFViewer {
  ...
  get currentScale() {
    return this._currentScale !== /* inlined export .UNKNOWN_SCALE */ 0
      ? this._currentScale
      : /* inlined export .DEFAULT_SCALE */ 1;
  }
    ...
    if (scale > 0) {
      ...
      switch (value) {
        ...
        case 'auto':
          ...
          scale = Math.min(
            /* inlined export .MAX_AUTO_SCALE */ 1.25,
            horizontalScale
          );
          break;
        default:
    ...
    if (!ignoreDestinationZoom) {
      if (scale && scale !== this._currentScale) {
        ...
        this.currentScaleValue =
          /* inlined export .DEFAULT_SCALE_VALUE */ 'auto';
      }
    }
  ...
  updateScale({ drawingDelay, scaleFactor = null, steps = null, origin }) {
    ...
    if (scaleFactor > 0 && scaleFactor !== 1) {
      ...
      const delta =
        steps > 0
          ? /* inlined export .DEFAULT_SCALE_DELTA */ 1.1
          : 1 / /* inlined export .DEFAULT_SCALE_DELTA */ 1.1;
    ...
    newScale = MathClamp(
      newScale,
      /* inlined export .MIN_SCALE */ 0.1,
      /* inlined export .MAX_SCALE */ 25
    );
