...
const EXTENSION_ICON_PATH =
  'M4.75 6.75h5.5L12 8.75h7.25v9.5H4.75V6.75ZM4.75 11.25h14.5';

export type WorkspaceFileSource = 'upload' | 'extension';
...
const fileOrigins = new WeakMap<
  File,
  { source: WorkspaceFileSource; sourceTabId?: number }
>();
...
export function markFileFromExtension(file: File, sourceTabId?: number): File {
  fileOrigins.set(file, { source: 'extension', sourceTabId });
  return file;
}
...
export function getExtensionFiles(): WorkspaceFileInfo[] {
  return currentFiles.filter((file) => file.source === 'extension');
}
...
function openFilesForSidebar(root: Document): WorkspaceFileInfo[] {
  return isHomePage(root) ? getExtensionFiles() : currentFiles;
}
...
function toFileInfo(
  ...
  if (file instanceof File) {
    ...
    return {
      name,
      size: file.size,
      source: origin?.source ?? 'upload',
      sourceTabId: origin?.sourceTabId,
...
function createFileButton(
  ...
  button.addEventListener('click', () => {
    if (file.source === 'extension') {
      void revealExtensionFile(file.sourceTabId);
      return;
    }
    openFilePicker(root);
  });
...
function createHomeFileRow(
  ...
  if (file.source === 'extension') {
    row.classList.add('is-revealable');
...
function activateHomeFile(file: WorkspaceFileInfo, root: Document): void {
  if (file.source === 'extension') {
    void revealExtensionFile(file.sourceTabId);
    return;
  }
...
function createFileIcon(
  ...
  svg.setAttribute(
    'class',
    source === 'extension'
      ? 'shift-nav-icon shift-open-file-icon-extension'
  ...
  path.setAttribute(
    'd',
    source === 'extension' ? EXTENSION_ICON_PATH : UPLOAD_ICON_PATH
  );
