...
describe('workspace files sidebar', () => {
  ...
  it('uses a tab icon for files opened through the Shift viewer', () => {
    mountShell();
    setWorkspaceFiles([
      { name: 'from-tab.pdf', size: 1024, source: 'extension', sourceTabId: 7 },
    ]);

    const button = document.querySelector('.shift-open-file-item');
    expect(button?.getAttribute('data-source')).toBe('extension');
    expect(button?.hasAttribute('title')).toBe(false);
    expect(button?.getAttribute('data-shift-tooltip')).toBe(
      'This PDF is open from a Shift tab. Click to show that tab.'
    );
    expect(button?.getAttribute('aria-label')).toBe(
      'from-tab.pdf. This PDF is open from a Shift tab. Click to show that tab.'
    );
    expect(
      button?.querySelector('.shift-open-file-icon-extension')
    ).not.toBeNull();
  ...
  it('tags a Shift-handoff File as an extension source', () => {
    mountShell();
    const file = new File([new Uint8Array([1, 2, 3])], 'from-tab.pdf', {
      type: 'application/pdf',
    });
    markFileFromExtension(file, 12);
  ...
  it('keeps the Shift origin when the file-input clones the File', () => {
    ...
    markFileFromExtension(original, 9);
  ...
  it('returns to the Shift viewer tab when an extension file is clicked', async () => {
    ...
    mountShell();
    setWorkspaceFiles([
      { name: 'from-tab.pdf', source: 'extension', sourceTabId: 7 },
    ]);
    const input = document.getElementById('file-input') as HTMLInputElement;
    const click = vi.spyOn(input, 'click').mockImplementation(() => {});
    ...
    await vi.waitFor(() => {
      expect(reveal).toHaveBeenCalledWith({ tabId: 7 });
    });
    expect(click).not.toHaveBeenCalled();
    click.mockRestore();
  });

  it('keeps the extension source when the in-page list refreshes the same name', async () => {
    mountShell();
    setWorkspaceFiles([
  ...
  it('reveals an extension file from the home thumbnail', async () => {
    const reveal = vi.fn().mockResolvedValue(undefined);
    Object.assign(window, {
      shift: { files: { read: vi.fn(), reveal } },
    });
    ...
    ).toContain('Click to show that tab');
    expect(
      document.querySelector<HTMLElement>('.shift-open-file-thumb-replace')
        ?.hidden
    ).toBe(true);
    pickerClick.mockRestore();
  });

  it('reveals an extension file from the home table', async () => {
    const reveal = vi.fn().mockResolvedValue(undefined);
    Object.assign(window, {
      shift: { files: { read: vi.fn(), reveal } },
    });
